---
title: "Nov2_MCmax30DMR_asinT_Stats"
author: "Shelly Trigg"
date: "11/02/2019"
...
#day 10
day10_STACKED_asin <- day10_STACKED
day10_STACKED_asin$perc.meth <- asinTransform(day10_STACKED_asin$perc.meth)

#arcsin transform data 
#day 135
day135_STACKED_asin <- day135_STACKED
day135_STACKED_asin$perc.meth <- asinTransform(day135_STACKED_asin$perc.meth)

#day 145
day145_STACKED_asin <- day145_STACKED
day145_STACKED_asin$perc.meth <- asinTransform(day145_STACKED_asin$perc.meth)

#all amb
amb_STACKED_asin <- amb_STACKED
amb_STACKED_asin$perc.meth <- asinTransform(amb_STACKED_asin$perc.meth)
```

plot distribution of TRANSFORMED % methylation in all DMRs in all samples
```{r}
jpeg("day10_MCmax30DMR_Tpercmeth_hist.jpg", width = 800, height = 800)
ggplot(day10_STACKED_asin) + geom_histogram(aes(perc.meth, group = group, color = group,fill = group), bins = 10, position = "identity", alpha = 0.5) + theme_bw()
dev.off()

jpeg("day135_MCmax30DMR_Tpercmeth_hist.jpg", width = 800, height = 800)
ggplot(day135_STACKED_asin) + geom_histogram(aes(perc.meth, group = group, color = group,fill = group), bins = 10, position = "identity", alpha = 0.5) + theme_bw()
dev.off()

jpeg("day145_MCmax30DMR_Tpercmeth_hist.jpg", width = 800, height = 800)
ggplot(day145_STACKED_asin) + geom_histogram(aes(perc.meth, group = group, color = group,fill = group), bins = 10, position = "identity", alpha = 0.5) + theme_bw()
dev.off()

jpeg("allAmb_MCmax30DMR_Tpercmeth_hist.jpg", width = 800, height = 800)
ggplot(amb_STACKED_asin) + geom_histogram(aes(perc.meth, group = group, color = group,fill = group), bins = 10, position = "identity", alpha = 0.5) + theme_bw()
dev.off()
```
...
```{r}
#for day 10 samples 
day10_aov = day10_STACKED_asin %>% group_by(ID) %>%
...
amb_aov = amb_STACKED_asin %>% group_by(ID) %>%
do(meth_aov_models = aov(perc.meth ~ group, data =  . ))
#summarize anova results
...
day135_aov = day135_STACKED_asin %>% group_by(ID) %>%
do(meth_aov_models = aov(perc.meth ~ group, data =  . ))
#summarize anova results
...
day145_aov = day145_STACKED_asin %>% group_by(ID) %>%
do(meth_aov_models = aov(perc.meth ~ group, data =  . ))
#summarize anova results
...
##Day10 ANOVA data
aov_0.1_d10_STACKED_asin <- day10_m[which(rownames(day10_m) %in% pull(day10_aov_modelsumm[which(day10_aov_modelsumm$p.value < 0.1),],ID)),]

ColSideColors <- cbind(pH = c(rep("cyan",4),rep("plum2",4),rep("magenta",4)))
jpeg("day10_MCmax30DMR_Taov0.1_heatmap.jpg", width = 800, height = 1000)
heatmap.2(aov_0.1_d10_STACKED_asin,margins = c(5,20), cexRow = 1.2, cexCol = 1,ColSideColors = ColSideColors, Colv=NA, col = bluered, na.color = "black", density.info = "none", trace = "none", scale = "row")
dev.off()

#look at the abundance plot

jpeg("day10_MCmax30DMR_Taov0.1_boxplots.jpg", width = 1000, height = 700)
ggplot(data = day10_STACKED_asin[which(day10_STACKED_asin$ID %in% pull(day10_aov_modelsumm[which(day10_aov_modelsumm$p.value < 0.1),],ID)),])+ geom_violin(aes(y = perc.meth,x = group, fill = group)) + facet_wrap(~ID, scale = "free") + theme_bw() + theme(axis.text.x = element_text(size = 7,angle = 45, hjust = 1),axis.title=element_text(size=12,face="bold"))
dev.off()

#DAY135 ANOVA data
aov_0.1_d135_STACKED_asin <- day135_m[which(rownames(day135_m) %in% pull(day135_aov_modelsumm[which(day135_aov_modelsumm$p.value < 0.1),],ID)),]

ColSideColors <- cbind(pH = c(rep("cyan",4),rep("plum2",4),rep("magenta",4)))
jpeg("day135_MCmax30DMR_Taov0.1_heatmap.jpg", width = 800, height = 1000)
heatmap.2(aov_0.1_d135_STACKED_asin,margins = c(5,20), cexRow = 1.2, cexCol = 1,ColSideColors = ColSideColors, Colv=NA, col = bluered, na.color = "black", density.info = "none", trace = "none", scale = "row")
dev.off()

#look at the abundance plot

jpeg("day135_MCmax30DMR_Taov0.1_boxplots.jpg", width = 1000, height = 700)
ggplot(data = day135_STACKED_asin[which(day135_STACKED_asin$ID %in% pull(day135_aov_modelsumm[which(day135_aov_modelsumm$p.value < 0.1),],ID)),])+ geom_violin(aes(y = perc.meth,x = group, fill = group)) + facet_wrap(~ID, scale = "free") + theme_bw() + theme(axis.text.x = element_text(size = 7,angle = 45, hjust = 1),axis.title=element_text(size=12,face="bold"))
dev.off()

#DAY145 ANOVA data
aov_0.1_d145_STACKED_asin <- day145_m[which(rownames(day145_m) %in% pull(day145_aov_modelsumm[which(day145_aov_modelsumm$p.value < 0.1),],ID)),]

ColSideColors <- cbind(pH = c(rep("cyan",4),rep("green1",4),rep("green3",4),rep("mediumpurple1",4),rep("plum2",4),rep("magenta",4)))
jpeg("day145_MCmax30DMR_Taov0.1_heatmap.jpg", width = 800, height = 1000)
heatmap.2(aov_0.1_d145_STACKED_asin,margins = c(5,20), cexRow = 1.2, cexCol = 1,ColSideColors = ColSideColors, Colv=NA, col = bluered, na.color = "black", density.info = "none", trace = "none", scale = "row")
dev.off()

#look at the abundance plot

jpeg("day145_MCmax30DMR_Taov0.1_boxplots.jpg", width = 1000, height = 700)
ggplot(data = day145_STACKED_asin[which(day145_STACKED_asin$ID %in% pull(day145_aov_modelsumm[which(day145_aov_modelsumm$p.value < 0.1),],ID)),])+ geom_violin(aes(y = perc.meth,x = group, fill = group)) + facet_wrap(~ID, scale = "free") + theme_bw() + theme(axis.text.x = element_text(size = 7,angle = 45, hjust = 1),axis.title=element_text(size=12,face="bold"))
dev.off()


# ALL AMB ANOVA data
aov_0.1_amb_STACKED_asin <- allAmb_m[which(rownames(allAmb_m) %in% pull(amb_aov_modelsumm[which(amb_aov_modelsumm$p.value < 0.1),],ID)),]

ColSideColors <- cbind(day = c(rep("#969696",4),rep("#737373",4),rep("#525252",4),rep("#252525",4)))
jpeg("amb_MCmax30DMR_Taov0.1_heatmap.jpg", width = 800, height = 1000)
heatmap.2(aov_0.1_amb_STACKED_asin,margins = c(5,20), cexRow = 1.2, cexCol = 1,ColSideColors = ColSideColors, Colv=NA, col = bluered, na.color = "black", density.info = "none", trace = "none", scale = "row")
dev.off()

#look at the abundance plot

jpeg("amb_MCmax30DMR_Taov0.1_boxplots.jpg", width = 1000, height = 700)
ggplot(data = amb_STACKED_asin[which(amb_STACKED_asin$ID %in% pull(amb_aov_modelsumm[which(amb_aov_modelsumm$p.value < 0.1),],ID)),])+ geom_violin(aes(y = perc.meth,x = group, fill = group)) + facet_wrap(~ID, scale = "free") + theme_bw() + theme(axis.text.x = element_text(size = 7,angle = 45, hjust = 1),axis.title=element_text(size=12,face="bold"))
dev.off()
```


HEATMAP OF GROUP MEANS (added 20200309)
```{r}

aov_0.1_amb_asin_means <- cbind(apply(aov_0.1_amb_STACKED_asin[,1:4],1,mean, na.rm= TRUE),apply(aov_0.1_amb_STACKED_asin[,5:8],1,mean, na.rm= TRUE),apply(aov_0.1_amb_STACKED_asin[,9:12],1,mean, na.rm= TRUE),apply(aov_0.1_amb_STACKED_asin[,13:16],1,mean, na.rm= TRUE))

colnames(aov_0.1_amb_asin_means) <- c("Day0", "Day10", "Day135", "Day145")

# ALL AMB ANOVA data
ColSideColors <- cbind(day = c(rep("#969696",1),rep("#737373",1),rep("#525252",1),rep("#252525",1)))

jpeg("amb_MCmax30DMR_Taov0.1_GROUPmean_heatmap2.jpg", width = 800, height = 1000)
heatmap.2(aov_0.1_amb_asin_means,margins = c(5,20), cexRow = 1.2, cexCol = 1,ColSideColors = ColSideColors, distfun = function(x) as.dist(1 - cor(t(x), use = "pa")), hclustfun = function(x) hclust(x,method = 'average'),Colv=NA, col= rev(colorRampPalette(brewer.pal(10, "RdYlBu"))(256)), na.color = "black", density.info = "none", trace = "none", scale = "row", labRow = FALSE,sepwidth=c(0.025,0.025),sepcolor="darkgray",colsep=1:ncol(aov_0.1_amb_asin_means),rowsep=1:nrow(aov_0.1_amb_asin_means))
dev.off()

jpeg("amb_MCmax30DMR_Taov0.1_GROUPmean_pheatmap.jpg", width = 800, height = 1000)
pheatmap(aov_0.1_amb_asin_means, clustering_method = "average",clustering_distance_rows="euclidean", show_rownames =FALSE,cluster_cols=FALSE,show_colnames =TRUE, scale = "row") 
dev.off()

###DAY 10
aov_0.1_d10_asin_means <- cbind(apply(aov_0.1_d10_STACKED_asin[,1:4],1,mean, na.rm= TRUE),apply(aov_0.1_d10_STACKED_asin[,5:8],1,mean, na.rm= TRUE),apply(aov_0.1_d10_STACKED_asin[,9:12],1,mean, na.rm= TRUE))

colnames(aov_0.1_d10_asin_means) <- c("ambient","low","superlow")

ColSideColors <- cbind(pH = c(rep("cyan",1),rep("plum2",1),rep("magenta",1)))


jpeg("d10_MCmax30DMR_Taov0.1_GROUPmean_heatmap2.jpg", width = 800, height = 1000)
heatmap.2(aov_0.1_d10_asin_means,margins = c(5,20), cexRow = 1.2, cexCol = 1,ColSideColors = ColSideColors,  hclustfun = function(x) hclust(x,method = 'average'),distfun = function(x) as.dist(1 - cor(t(x), use = "pa")),Colv=NA, col= rev(colorRampPalette(brewer.pal(10, "RdYlBu"))(256)), na.color = "black", density.info = "none", trace = "none", scale = "row", labRow = FALSE,sepwidth=c(0.025,0.025),sepcolor="darkgray",colsep=1:ncol(aov_0.1_d10_asin_means),rowsep=1:nrow(aov_0.1_d10_asin_means))
dev.off()



aov_0.1_d135_asin_means <- cbind(apply(aov_0.1_d135_STACKED_asin[,1:4],1,mean, na.rm= TRUE),apply(aov_0.1_d135_STACKED_asin[,5:8],1,mean, na.rm= TRUE),apply(aov_0.1_d135_STACKED_asin[,9:12],1,mean, na.rm= TRUE))

colnames(aov_0.1_d135_asin_means) <- c("ambient", "low", "superlow")

ColSideColors <- cbind(pH = c(rep("cyan",1),rep("plum2",1),rep("magenta",1)))


jpeg("d135_MCmax30DMR_Taov0.1_GROUPmean_heatmap2.jpg", width = 800, height = 1000)
heatmap.2(aov_0.1_d135_asin_means,margins = c(5,20), cexRow = 1.2, cexCol = 1,ColSideColors = ColSideColors, distfun = function(x) as.dist(1 - cor(t(x), use = "pa")), hclustfun = function(x) hclust(x,method = 'average'),Colv=NA, col= rev(colorRampPalette(brewer.pal(10, "RdYlBu"))(256)), na.color = "black", density.info = "none", trace = "none", scale = "row", labRow = FALSE,sepwidth=c(0.025,0.025),sepcolor="darkgray",colsep=1:ncol(aov_0.1_d135_asin_means),rowsep=1:nrow(aov_0.1_d135_asin_means))
dev.off()


aov_0.1_d145_asin_means <- cbind(apply(aov_0.1_d145_STACKED_asin[,1:4],1,mean, na.rm= TRUE),apply(aov_0.1_d145_STACKED_asin[,13:16],1,mean, na.rm= TRUE),apply(aov_0.1_d145_STACKED_asin[,5:8],1,mean, na.rm= TRUE),apply(aov_0.1_d145_STACKED_asin[,17:20],1,mean, na.rm= TRUE),apply(aov_0.1_d145_STACKED_asin[,9:12],1,mean, na.rm= TRUE),apply(aov_0.1_d145_STACKED_asin[,21:24],1,mean, na.rm= TRUE))
colnames(aov_0.1_d145_asin_means) <-c("ambient","amb_low","low_amb","low_low","superlow_amb","superlow_low")


ColSideColors <- cbind(pH = c(rep("cyan",1),rep("green1",1),rep("green3",1),rep("mediumpurple1",1),rep("plum2",1),rep("magenta",1)))


jpeg("d145_MCmax30DMR_Taov0.1_GROUPmean_heatmap2.jpg", width = 800, height = 1000)
heatmap.2(aov_0.1_d145_asin_means,margins = c(5,20), cexRow = 1.2, cexCol = 1,ColSideColors = ColSideColors, distfun = function(x) as.dist(1 - cor(t(x), use = "pa")), hclustfun = function(x) hclust(x,method = 'average'),Colv=NA, col= rev(colorRampPalette(brewer.pal(10, "RdYlBu"))(256)), na.color = "black", density.info = "none", trace = "none", scale = "row", labRow = FALSE,sepwidth=c(0.025,0.025),sepcolor="darkgray",colsep=1:ncol(aov_0.1_d145_asin_means),rowsep=1:nrow(aov_0.1_d145_asin_means))
dev.off()
```
